...
const columns: Array<CadastroColumn<AnimalRecord> & { key: string }> = [
...
export default function AnimaisPage({
  ...
  const fields: CadastroField<AnimalRecord>[] = [
  ...
  const getSupplierName = (supplierId: string) =>
    suppliers.find((supplier) => supplier.id === supplierId)?.companyName || 'Fornecedor não informado';
    ...
    <CadastroSection
      ...
      columns={columns.map((column) =>
        column.key === 'supplierId'
          ? {
              ...column,
              render: (record: AnimalRecord) => getSupplierName(record.supplierId),
            }
          : column,
      )}
          ...
          <div className="space-y-3">
            ...
            <div className="grid gap-3 md:grid-cols-3">
              <div className="rounded-2xl border border-gray-200 bg-white p-3">
                ...
                <div className="mt-1 text-sm font-extrabold text-[#0f1c2b]">{getSupplierName(draft.supplierId)}</div>
              </div>
