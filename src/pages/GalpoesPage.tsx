...
export default function GalpoesPage({
  ...
  const fields: CadastroField<GalpaoRecord>[] = [
    ...
    { key: 'mortalityThresholdPercent', label: 'Limite de Mortalidade (%)', type: 'number', min: 0, max: 100 },
  ];

  const getLotsFromNotes = (notes: string): LoteSelecionado[] => {
    try {
  ...
  const calculateCurrentBirdCount = (lotes: LoteSelecionado[]) => {
    ...
    <CadastroSection
      ...
      getSummary={(items) => [
        { label: 'Galpões', value: String(items.length) },
        {
          ...
          value: String(items.reduce((total, item) => total + item.capacity, 0)),
        },
        {
          label: 'Aves Alocadas',
          value: String(items.reduce((total, item) => total + item.currentBirdCount, 0)),
        },
        {
          ...
          value:
            items.length > 0
              ? `${Math.round(
                  (items.reduce((total, item) => total + item.currentBirdCount, 0) /
                    items.reduce((total, item) => total + item.capacity, 0)) *
                    100
                )}%`
              : '0%',
        },
                  ...
                  >
                    <div>
                      ...
                      <p className="text-xs text-gray-500">
                        {lote.currentQuantity ?? lote.quantity} aves
                      </p>
                    </div>
                    <button
                      type="button"
